...
class PostValidationPlugin(Generic[TInput, TTarget, TOutput, TModel, TTrainer]):
    @abstractmethod
...
class PreValidationPlugin(Generic[TInput, TTarget, TOutput, TModel, TTrainer]):
    @abstractmethod
    def pre_validation(self, logger: Logger, training_context: TrainingContext[TInput, TTarget, TOutput, TModel, TTrainer]):
        pass

ValidationPlugins = Union[PreValidationPlugin, PostValidationPlugin]

class ValidationPlugin(PostEpoch[TInput, TTarget, TOutput, TModel, TTrainer]):
    def __init__(self, evaluation_service: EvaluationService[TInput, TTarget, TOutput, TModel], validation_datasets: EvaluationDataset, validation_metrics: EvaluationMetrics, event_loop: asyncio.AbstractEventLoop = None, plugins: Dict[str, ValidationPlugins] = {}):
        ...
        self.__evaluation_service: EvaluationService[TInput, TTarget, TOutput, TModel] = evaluation_service
        ...
        self.__pre_validation_plugins: Dict[str, PreValidationPlugin[TInput, TTarget, TOutput, TModel, TTrainer]] = dict(filter(lambda plugin: isinstance(plugin[1], PreValidationPlugin), plugins.items()))
        self.__post_validation_plugins: Dict[str, PostValidationPlugin[TInput, TTarget, TOutput, TModel, TTrainer]] = dict(filter(lambda plugin: isinstance(plugin[1], PostValidationPlugin), plugins.items()))

    def __execute_pre_validation_plugins(self, logger: Logger, context: TrainingContext[TInput, TTarget, TOutput, TModel, TTrainer]):
    ...
    def __execute_post_validation_plugins(self, logger: Logger, context: TrainingContext[TInput, TTarget, TOutput, TModel, TTrainer], validation_result: EvaluationResult):
    ...
    def post_epoch(self, logger: Logger, trainingContext: TrainingContext[TInput, TTarget, TOutput, TModel, TTrainer]):
