...
class StopCondition(Generic[TInput, TTarget, TOutput, TModel, TTrainer], ABC):
    @abstractmethod
    def reset(self):
        pass

    @abstractmethod
    def is_satisfied(self, logger: Logger, training_context: TrainingContext[TInput, TTarget, TOutput, TModel, TTrainer], validation_result: EvaluationResult) -> bool:
        pass

StopConditions = Dict[str, StopCondition[TInput, TTarget, TOutput, TModel, TTrainer]]

class EarlyStoppingPlugin(PostValidationPlugin[TInput, TTarget, TOutput, TModel, TTrainer]):
